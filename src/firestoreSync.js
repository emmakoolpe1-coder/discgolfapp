...
import { getDiscSyncBlockReason } from './firestoreSyncGuards.js';
...
export async function syncToFirestore(userId, discs, bags, aces, tournaments, longestThrows, personalBests, dataLoaded = false, skillLevel, throwStyle) {
  ...
  try {
    ...
    const discBlockReason = getDiscSyncBlockReason({
      incomingDiscIds: discsList.map((d) => d?.id),
      remoteDiscIds: currentDiscSnap.docs.map((s) => s.id),
      dataLoaded,
    });

    // Guard only pre-load/stale writes. Once Firestore has loaded, user deletions are intentional.
    if (discBlockReason) {
      console.warn('[sync] ⚠️ BLOCKED:', discBlockReason);
      return false;
    }
