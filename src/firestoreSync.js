...
export async function syncToFirestore(userId, discs, bags, aces, tournaments, longestThrows, personalBests, dataLoaded = false, skillLevel, throwStyle) {
  ...
  try {
    ...
    // CRITICAL: Before load, local state can be empty/default. After load, lower
    // counts are intentional user deletions and must be allowed to sync.
    const preloadBlockReason = getPreloadDiscSyncBlockReason(discsList.length, remoteDiscCount, dataLoaded);
    if (preloadBlockReason) {
      console.warn('[sync] ⚠️ BLOCKED:', preloadBlockReason);
      return false;
    }
